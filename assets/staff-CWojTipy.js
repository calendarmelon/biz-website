import{d as s,r as c}from"./index-C8EDacUC.js";const f=async e=>{try{const{data:r,error:o}=await s.from("employees").select("*").eq("user_id",e).eq("status","active").single();if(o)throw o;return{data:r,error:null}}catch(r){return{data:null,error:r.message}}},w=async(e,r={})=>{try{let o=s.from("employees").select(`
        *,
      ...
      `).eq("employee_id",e).gte("appointment_date",r).lte("appointment_date",o).order("appointment_date",{ascending:!0}).order("start_time",{ascending:!0});if(n)throw n;return{data:t,error:null}}catch(t){return{data:null,error:t.message}}},q=async(e,r,o)=>{try{const{data:t,error:n}=await s.from("appointments").select(`
        ...
        total_amount,
        appointment_date
      `).eq("employee_id",e).gte("appointment_date",r).lte("appointment_date",o);if(n)throw n;const a=t.length,i=t.filter(l=>l.status==="completed").length,d=t.reduce((l,u)=>u.status==="completed"?l+parseFloat(u.total_amount):l,0);return{data:{total_appointments:a,completed_appointments:i,total_revenue:d,completion_rate:a>0?i/a*100:0},error:null}}catch(t){return{data:null,error:t.message}}};export{b as a,f as b,v as c,y as d,C as e,q as f,w as g,h,R as r,E as u,g as v};
